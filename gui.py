...
textAccuracy=ft.Text("    Accuracy:\n         ???",      style=ft.TextStyle(size=16,weight=ft.FontWeight.BOLD, color=ft.colors.BLACK))
...
async def main(page: ft.Page):
    ...
    page.title = "Healthy Or Diseased ???"
    ...
    def accuracy():
      ...
      # counter = 0
      # for i in range(1,5):
      #     img = cv2.imread(f"test/{i}.jpeg",cv2.IMREAD_GRAYSCALE)
      #     resized = cv2.resize(img, (300, 300), interpolation = cv2.INTER_AREA)
      #     p = np.array(flatten(resized) )
      #     p = p.transpose()
      #     a = np.dot(weights, p) 
          
      #     counter += 1 if (a.argmax() % 2 == 0 and i <= 2) or (a.argmax() % 2 == 1 and i > 2) else 0
